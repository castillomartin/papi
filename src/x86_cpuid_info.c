...
static unsigned int
cpuid3 (unsigned int eax, char *sig)
{
...
vm_cpu_sig (void)
{
  ...
  max_entries = cpuid3 (leaf, sig);
  if (strncmp(sig,"VBox",sizeof(vbox)) == 0) {
    strncpy(vbox,sig,sizeof(vbox));
    return vbox;
  }

  if (max_entries < 3 || max_entries > 0x10000) {
    //    printf("max_entries bounds %x %s %d\n",leaf,sig,max_entries);
    strncpy(vbox,sig,sizeof(vbox));
    return vbox;
  }
...
_x86_detect_hypervisor(char *vendor_name)
{
  ...
  if  (ecx&0x80000000) {
    ...
    char *r = vm_cpu_sig();
    if (r) {
      strncpy(vendor_name,r,PAPI_MAX_STR_LEN);
      return 1;
    }
  } 
